    ...
    <nav className="sticky top-0 z-50 w-full py-4 px-4 md:px-8 lg:px-12 flex items-center justify-between
      ...
      <div className="md:hidden flex items-center">
        <button
          ...
          className="ml-4 text-darkText dark:text-lightText focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-accent rounded-md p-2"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-darkBg/95 dark:bg-lightBg/95 z-40 backdrop-blur-lg md:hidden"
          >
            {/* Close Button outside GlassCard */}
            <button
              onClick={toggleMobileMenu}
              className="absolute top-4 right-4 text-gray-500 hover:text-accent focus:outline-none focus:ring-2 focus:ring-accent rounded-full p-2 z-50 bg-darkBg/70 dark:bg-lightBg/70 backdrop-blur-md"
            ...
            <GlassCard className="absolute top-16 right-4 p-4 w-[calc(100%-2rem)] max-w-sm space-y-3 flex flex-col items-center">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <NavLink
                    key={link.name}
                    to={link.path}
                    className="w-full text-center py-2 text-lg text-darkText dark:text-lightText hover:text-accent dark:hover:text-accent transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-accent rounded-md"
                    onClick={toggleMobileMenu}
                  >
                    {link.name}
                  </NavLink>
              ...
              <div className="w-full h-px bg-gray-700 dark:bg-gray-300 my-2"></div>
              {isAuthenticated ? (
                 <>
