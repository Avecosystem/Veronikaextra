...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const handleOxapayReturn = async () => {
      ...
      // Check for various status indicators Oxapay might send
      ...
      if (orderId && oxapayStatus && !verifyingPayment) {
        ...
        try {
          ...
          if (oxapayStatus === 'waiting') {
             setVerificationMessage({ type: 'error', text: 'Payment is still processing. Please check back later.' });
             setVerifyingPayment(false);
             return;
          }

          // Cast to expected type. ProfilePage logic maps unknown states safely.
          const statusForBackend = (oxapayStatus === 'paid' || oxapayStatus === 'success' || oxapayStatus === 'confirming') 
             ? 'success' 
             : (oxapayStatus as 'success' | 'failed' | 'cancelled');

          const response: ApiResponse<{ newCredits: number }> = await backendApi.verifyOxapayPayment(
            token, 
            orderId, 
            statusForBackend
          );
          ...
          if (response.success) {
            setVerificationMessage({ type: 'success', text: `Crypto payment successful! Your new balance is ${response.data.newCredits} credits.` });
            updateUserCredits(response.data.newCredits);
          } else {
            setVerificationMessage({ type: 'error', text: `Crypto payment verification: ${response.message || 'Pending or Failed.'}` });
          }
        } catch (err) {
          console.error('Error verifying OXPAY payment:', err);
          ...
          >
             {verifyingPayment && (
                 <GlassCard className="p-4 text-center flex items-center justify-center bg-blue-500/20 border-blue-500/50">
                     ...
                     <p className="text-blue-500 font-semibold">Verifying your crypto payment...</p>
                 </GlassCard>
             )}
